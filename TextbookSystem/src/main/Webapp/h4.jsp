...
<ul>
  ...
  <li><a   class="active" href="Pending2Servlet">进书通知书</a></li>
...
<table frame="below" border=1>
  ...
  <tbody>
  <c:forEach items="${list}" var="in_storage">
    <tr>
      <td>${in_storage.id}</td>
      <td>${in_storage.book_id}</td>
      <td>${in_storage.book_name}</td>
      <td>${in_storage.price}</td>
      <td>${in_storage.number}</td>
    </tr>
  </c:forEach>
...
<div class="divbutton">
  <center>
    <a class="btn1" href="delectServlet">已阅</a><br>
    第${paging.page+1}页/共${paging.pagenumber}页<br>
    <a class="previous round" href="Pending2Servlet?page=${paging.indexpage-1}">&#8249;首页</a>
    <a  class="previous" href="Pending2Servlet?page=${paging.page-1 }">&laquo; 上一页</a>
    <a class="next" href="Pending2Servlet?page=${paging.page+1}">下一页 &raquo;</a>
    <a class="next round" href="Pending2Servlet?page=${paging.pagenumber-1}">&#8250;末页</a>
  </center>
</div>
